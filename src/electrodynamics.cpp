...
Electrodynamics::Electrodynamics(Times time) {
  ...
  read_netcdf_electrodynamics_file(get_electrodynamics_file());
...
void Electrodynamics::set_time(double time) {
  ...
  if (intime < times[iLow]) {
    ...
    print(0, "Warning: current time below first available\n");
    print(0, "potential-vector time, using first time\n");
  } else if (intime > times[iHigh]) {
    interpolation_index = iHigh;
    print(0, "Warning: current time above last available\n");
    print(0, "potential-vector time, using last time\n");
  }
