...
int main() {
  ...
  try {
    ...
    if (get_is_student())
      print(-1, "Hello " +
		   get_student_name() + " - welcome to Aether!");
    ...
    if (get_do_restart()) {
      print(1, "Restarting! Reading time file!");
      DidWork = time.restart_file(get_restartin_dir(), DoRead);
      if (!DidWork)
	throw std::string("Reading Restart for time Failed!!!\n");
    }

    // This is for the initial output.  If it is not a restart, this will go:
    if (time.check_time_gate(get_dt_output(0)))
	...
	DidWork = neutrals.restart_file(get_restartout_dir(), DoWrite);
	if (!DidWork)
	  throw std::string("Writing Restart for Neutrals Failed!!!\n");	

	DidWork = ions.restart_file(get_restartout_dir(), DoWrite);
	if (!DidWork)
	  throw std::string("Writing Restart for Ions Failed!!!\n");	

	DidWork = time.restart_file(get_restartout_dir(), DoWrite);
	if (!DidWork)
